	...
	for k, v := range projectObj {
		...
		for k, v := range phaseObj {
			if arr, ok := v.([]interface{}); ok {
				...
				phaseID := AddPhase(phaseNum, project.ID, k, int(arr[0].(float64)), int(arr[1].(float64)), int(arr[2].(float64)), arr[3].([]interface{}), arr[5].([]interface{}), arr[4].([]interface{}), arr[6].([]interface{}), arr[8].([]interface{}), arr[7].([]interface{}))
