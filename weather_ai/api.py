...
def generate_predictions():
    try:
        ...
        historical_data = pd.read_csv('weather_ai/preprocessed_data.csv', parse_dates=['Date'], index_col='Date')
        ...
        loaded_model = tf.keras.models.load_model('weather_ai/custom_weather_data.keras')
        ...
        date_range_2024 = pd.date_range(start='2024-01-01', periods=steps_to_predict, freq='D')
        predicted_df = pd.DataFrame(predicted_values, index=date_range_2024, columns=['Predicted AvgTemperature'])
        
        # Convert the DataFrame to a CSV string
        csv_string = predicted_df.to_csv(index_label='Date')
        
        # Create a Flask Response with the CSV string
        response = Response(csv_string, mimetype='text/csv')
        response.headers.set("Content-Disposition", "attachment", filename="test_predictions_2024.csv")
